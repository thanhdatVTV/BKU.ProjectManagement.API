...
{
    public class AppLecturerConfiguration : IEntityTypeConfiguration<AppLecturer>
    {
        public void Configure(EntityTypeBuilder<AppLecturer> builder)
        {
            builder.ToTable("AppLecturer");
            ...
            builder.HasIndex(x => x.SsoTeacherId).IsUnique();
            builder.HasIndex(x => x.TeacherCode).IsUnique();
            builder.HasIndex(x => x.AppUserId).IsUnique();

            builder.HasOne(x => x.AppUser)
                .WithOne(x => x.AppLecturer)
                .HasForeignKey<AppLecturer>(x => x.AppUserId)
                .OnDelete(DeleteBehavior.Restrict);

            builder.HasOne(x => x.Course)
                .WithMany(x => x.AppLecturers)
